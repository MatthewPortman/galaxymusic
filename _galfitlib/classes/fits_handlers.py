...
class FitsFile:
    def __init__(self,
        ...
        self.gname    = kwargs.get("gname", Path(filepath).name.split("_galfit_out.fits")[0])
        
        assert Path(filepath).suffix.lower() == ".fits", "File being passed into FitsHandler must be .fits!"
...
# ==========================================================================================================
    ...
    def to_png(self, cleanup = True, **kwargs): #tmp_fits_path = "", tmp_png_path = "", out_filename = ""):
        ...
        # .../galfits -> galfit_png
        #tmp_png_dir   = Path(tmp_fits_path)[0].rstrip("s") + "_png"
...
# ==========================================================================================================
    ...
    def generate_bulge_mask(self, sparcfire_csv):
        ...
        try:
            ...
            print(e)
            print(self.gname)
            print(center_x, center_y, major_rad, minor_rad, bulge_angle)
            return bulge_mask
...
# ==========================================================================================================
    ...
    def generate_masked_residual(
        ...
        feedme_path             = Path(self.feedme.path_to_feedme)
        feedme_dir, feedme_file = feedme_path.parent, feedme_path.name
...
if __name__ == "__main__":
    ...
    print("\nNow with bulge mask")
    _ = test_model.generate_bulge_mask(pj(TEST_DATA_DIR, "out", gname, f"{gname}.csv"))
...
if __name__ == "__main__":
    export_to_py("fits_handlers", pj(LIBRARY_DIR, "classes", "fits_handlers"))
